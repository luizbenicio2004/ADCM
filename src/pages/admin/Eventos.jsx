...
const VAZIO = { titulo: "", descricao: "", data: "", horario: "", local: "", tipo: "evento", ativo: true, fotoUrl: "", fotos: [] };
...
export default function AdminEventos() {
  ...
  const handleGaleria = async (e) => {
    ...
    try {
      const urls = await Promise.all(files.map((f) => upload(f, `eventos/galeria/${Date.now()}_${f.name}`)));
  ...
  function handleEditar(evento) {
    setForm({
      titulo: evento.titulo ?? "", descricao: evento.descricao ?? "", data: evento.data ?? "",
      horario: evento.horario ?? "", local: evento.local ?? "", tipo: evento.tipo ?? "evento",
      ativo: evento.ativo ?? true, fotoUrl: evento.fotoUrl ?? "", fotos: evento.fotos ?? [] });
      ...
      <div className="max-w-3xl flex flex-col gap-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 flex flex-col gap-5">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              ...
              <input name="titulo" value={form.titulo} onChange={handleChange} placeholder="Ex: Culto de Aniversário"
          ...
          <div className="flex flex-col gap-1">
            ...
            <textarea name="descricao" rows={3} value={form.descricao} onChange={handleChange} placeholder="Descreva o evento..."
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input name="horario" value={form.horario} onChange={handleChange} placeholder="Ex: 19h30"
                className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-900/30" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Local</label>
              <input name="local" value={form.local} onChange={handleChange} placeholder="Ex: Templo Principal"
                className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-900/30" />
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={handleSubmit} disabled={salvando || uploading}
              ...
              <Check size={16} /> {salvando ? "Salvando…" : editandoId ? "Salvar alterações" : "Criar evento"}
            </button>
            {editandoId && (
        ...
        <div className="flex flex-col gap-3">
          ...
          {loading ? (
            ...
            <p className="text-sm text-gray-500 text-center py-8">Nenhum evento por enquanto. Que tal adicionar o primeiro?</p>
          ) : (
            eventos.map((ev) => (
              <div key={ev.id} className="bg-white border border-gray-200 rounded-xl p-4 flex items-start gap-4">
                {ev.fotoUrl ? (
                  <OptimizedImage src={ev.fotoUrl} alt={ev.titulo} className="w-16 h-16 object-cover rounded-lg flex-shrink-0" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                      ev.tipo === "urgente" ? "bg-red-100 text-red-700" :
                      ev.tipo === "aviso"   ? "bg-yellow-100 text-yellow-700" :
                      "bg-blue-100 text-blue-700"}`}>{ev.tipo}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${ev.ativo ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                      {ev.ativo ? "publicado" : "oculto"}
                    </span>
                    {(ev.fotos ?? []).length > 0 && (
                      <span className="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full">🖼 {ev.fotos.length} fotos</span>
                    )}
                  </div>
                  <h3 className="font-semibold text-gray-900 text-sm">{ev.titulo}</h3>
                  <p className="text-xs text-gray-500 mt-0.5">{ev.data} {ev.horario} {ev.local && `· ${ev.local}`}</p>
                </div>
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button onClick={() => handleEditar(ev)} className="p-2 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition"><Pencil size={15} /></button>
                  <button onClick={() => setConfirm({ open: true, id: ev.id })} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition"><Trash2 size={15} /></button>
                </div>
              </div>
            ))
          )}
