...
import { ArrowLeft, Plus, Pencil, Trash2, X, Check, ImagePlus, Loader2, Calendar } from "lucide-react";

const VAZIO = {
  titulo: "", descricao: "", data: "",
  horario: "", local: "", tipo: "evento", ativo: true,
  fotoUrl: "", fotos: [],
};
...
export default function AdminEventos() {
  ...
  async function handleRemover(id) {
    if (confirm("Tem certeza que deseja excluir este evento?")) await remover(id);
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center gap-4">
        ...
        <div>
          <h1 className="font-bold text-gray-900">Gerenciar Eventos</h1>
          <p className="text-xs text-gray-500">Crie e gerencie os eventos da igreja</p>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-6 py-8 flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-3">
          <h2 className="font-semibold text-gray-800">Eventos cadastrados</h2>
          {loading ? (
            Array(3).fill(0).map((_, i) => <div key={i} className="h-20 bg-white rounded-xl border border-gray-200 animate-pulse" />)
          ) : eventos.length === 0 ? (
            <p className="text-sm text-gray-500 text-center py-8">Nenhum evento cadastrado ainda.</p>
          ) : (
            eventos.map((ev) => (
              <div key={ev.id} className="bg-white border border-gray-200 rounded-xl p-4 flex items-start gap-4">
                {ev.fotoUrl ? (
                  <img src={ev.fotoUrl} alt={ev.titulo} className="w-16 h-16 object-cover rounded-lg flex-shrink-0" />
                ) : (
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <button onClick={() => handleRemover(ev.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition"><Trash2 size={15} /></button>
                </div>
              </div>
            ))
          )}
