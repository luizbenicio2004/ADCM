//    — padroniza com todos os outros admins (Eventos, Avisos).
...
export default function AdminMinisterios() {
  ...
  const handleGaleria = async (e) => {
    ...
    try {
      const urls = await Promise.all(
        files.map((f) => upload(f, `ministerios/galeria/${Date.now()}_${f.name}`))
      );
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.youtubeUrl && !isValidYoutubeUrl(form.youtubeUrl)) {
      setErro("URL do YouTube inválida. Use o formato: https://www.youtube.com/watch?v=...");
      return;
    }
    setErro("");
    setSalvando(true);
    try {
      if (editandoId) {
        ...
        addToast("Ministério atualizado!");
      } else {
        await adicionar(form);
        addToast("Ministério criado!");
      }
      setForm(VAZIO);
      setEditandoId(null);
    } catch {
  ...
  return (
    <AdminLayout
      ...
      subtitle="Fotos, vídeos e história de cada ministério"
    >
      <div className="max-w-3xl flex flex-col gap-8">
        <div className="bg-white rounded-xl border border-gray-200 p-6 flex flex-col gap-5">
          <h2 className="font-semibold text-gray-800">
            {editandoId ? "✏️ Editar Ministério" : "➕ Novo Ministério"}
          </h2>
          ...
          {/* CORRIGIDO: validação de URL do YouTube */}
          <div className="flex flex-col gap-1">
          ...
          {/* CORRIGIDO: id único "ministerio-ativo" */}
          <div className="flex items-center gap-2">
            <input
              ...
              className="rounded"
            />
            <label htmlFor="ministerio-ativo" className="text-sm text-gray-700">
          ...
          <div className="flex gap-3 pt-1">
            ...
            >
              <Check size={16} />{" "}
              {salvando ? "Salvando…" : editandoId ? "Salvar alterações" : "Criar ministério"}
            </button>
            {editandoId && (
              <button
        ...
        <div className="flex flex-col gap-3">
          ...
          {loading ? (
            ...
            <p className="text-sm text-gray-500 text-center py-8">
              Nenhum ministério adicionado ainda.
            </p>
          ) : (
            ministerios.map((m) => (
              <div
