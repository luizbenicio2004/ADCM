...
export default function AdminLive() {
  ...
  const handleSave = async (e) => {
    ...
    // Validação da URL do YouTube antes de salvar
    if (liveUrl && !isValidYoutubeUrl(liveUrl)) {
      setErro("URL inválida. Use uma URL do YouTube válida: https://www.youtube.com/watch?v=...");
      return;
    }
  ...
  // Usa a função centralizada com autoplay para o preview
  const embedUrl = toEmbedUrl(liveUrl, { autoplay: 1 });
  const embedPreview = toEmbedUrl(liveUrl); // preview sem autoplay

  return (
    <AdminLayout
      title="Transmissão ao Vivo"
      subtitle="Ative e configure a live do YouTube no site"
    >
        ...
        {loading ? (
          ...
          <form onSubmit={handleSave} className="flex flex-col gap-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 flex flex-col gap-5">
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    Quando ativado, o banner e o player aparecem na home
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  aria-label={liveAtivo ? "Desativar live" : "Ativar live"}
                  className={`relative w-12 h-6 rounded-full transition-colors ${
                    liveAtivo ? "bg-red-500" : "bg-gray-300"
                  }`}
                >
                  <span
                    className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-transform ${
                      liveAtivo ? "translate-x-7" : "translate-x-1"
                    }`}
                  />
                </button>
              </div>
              ...
              {/* Preview — usa embedPreview (sem autoplay para não tocar ao abrir o admin) */}
              {embedPreview && (
