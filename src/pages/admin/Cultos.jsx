...
// Converte HH:MM → "19h30" para exibição
...
export default function AdminCultos() {
  ...
  // CORRIGIDO: usa writeBatch em vez de N+1 updateDoc individuais
  const tornarPrincipal = async (cultoId) => {
    try {
      const batch = writeBatch(db);
      cultos.forEach((c) => {
        batch.update(doc(db, "cultos", c.id), { destaque: c.id === cultoId });
      });
      await batch.commit();
      addToast("Culto principal atualizado!");
    } catch {
      addToast("Erro ao atualizar culto principal.", "error");
    }
  };
      ...
      <div className="max-w-3xl flex flex-col gap-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            {/* CORRIGIDO: input type="time" para padronizar formato de horário */}
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">
                Horário *
              </label>
            ...
            <div className="flex flex-col gap-1 sm:col-span-2">
              ...
              <input
                type="text"
                name="obs"
                placeholder="Ex: Quinzenal — confirme pelas redes sociais"
                value={form.obs}
                onChange={handleChange}
                className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-900/30"
              />
        ...
        <div className="flex flex-col gap-3">
          ...
          {loading ? (
            ...
            <p className="text-sm text-gray-500 text-center py-8">
              Nenhum culto adicionado ainda. Comece pelo botão acima!
            </p>
          ) : (
            cultos.map((culto) => (
              <div
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-gray-900 text-sm">
                      {culto.nome || culto.dia}
                    </span>
                  </div>
                  <p className="text-sm text-gray-500">
                    {culto.dia} —{" "}
                    <span className="font-bold text-blue-900">
                      {/* Exibe formato amigável se vier do novo campo time */}
                      {culto.horario?.includes(":")
                        ? formatarHorario(culto.horario)
                        : culto.horario}
                    </span>
                  </p>
                  {culto.descricao && (
                    <p className="text-xs text-gray-400 mt-0.5">{culto.descricao}</p>
                  )}
                  {culto.obs && (
                    <p className="text-xs text-amber-600 mt-0.5">⚠️ {culto.obs}</p>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  {!culto.destaque && (
                    <button
                      onClick={() => tornarPrincipal(culto.id)}
                      title="Tornar principal"
                      className="p-2 text-gray-400 hover:text-blue-900 hover:bg-blue-50 rounded-lg transition min-w-[40px] min-h-[44px] flex items-center justify-center"
                    >
                      <Star size={16} />
                    </button>
                  )}
                  <button
                    onClick={() => iniciarEdicao(culto)}
                  ...
                  <button
                    onClick={() => setConfirm({ open: true, id: culto.id })}
